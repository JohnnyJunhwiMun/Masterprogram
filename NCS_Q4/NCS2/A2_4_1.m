...
F_over_approx{2}=double(subs(F0+min_a1*F1+max_a2*F2,h,h_val));
F_over_approx{3}=double(subs(F0+min_a1*F1+min_a2*F2,h,h_val));
F_over_approx{4}=double(subs(F0+max_a1*F1+min_a2*F2,h,h_val));
...
G_over_approx{2}=double(subs(G0+min_a1*G1+max_a2*G2,h,h_val));
G_over_approx{3}=double(subs(G0+min_a1*G1+min_a2*G2,h,h_val));
G_over_approx{4}=double(subs(G0+max_a1*G1+min_a2*G2,h,h_val));

hold on 
for i=1:1:length(G_over_approx)
scatter(G_over_approx{i}(1),G_over_approx{i}(2),'green','filled');hold on
