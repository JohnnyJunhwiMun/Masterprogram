...
% variables
u = sdpvar(2, Tfinal, 4); % u(i,t,k) represents the control input of aircraft k at time t
x = sdpvar(4, Tfinal+1, 4); % x(:,t,k) represents the state of aircraft k at time t
xf = sdpvar(4, 1); % Common terminal state

% Define objective function
obj = 0;
for k = 1:4
    obj = obj + sum(sum(x(:,1:Tfinal,k).^2)) + sum(sum(u(:,:,k).^2));
end
...
for k = 1:4
    for t = 1:Tfinal
        lagrangian = lagrangian + (x(:,t+1,k) - A{k}*x(:,t,k) - B{k}*u(:,t,k) - xf)'*lambda(:,t);
    end
end
obj = obj + lagrangian;
...
for iteration = 1:max_iterations
    % Fix xf to the previous iteration's value
    constr = [constr, xf == x_hist(:,1,1)];
...
figure;
plot(0:max_iterations, error_seq);
